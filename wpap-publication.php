<?php
	...
	function wpap_get_pubs_formatted ($options) {
		...
		foreach ($pubs as $pub) {
			...
			if (strtolower($options['show_links']) == 'true') {
				if (!empty($pub['pdf_url'])) {
					$link = '<a class="publication-button publication elementor-button button" href="' . $pub['pdf_url'] . '">' . __('PDF', 'wpap') . '</a>';
					array_push($links, $link);
				}
				if (!empty($pub['bibtex_url'])) {
					$link = '<a class="publication-button publication elementor-button button" href="' . $pub['bibtex_url'] . '">' . __('BibTex', 'wpap') . '</a>';
					array_push($links, $link);
				}
				if (!empty($pub['slides_ppt'])) {
					$link = '<a class="publication-button publication elementor-button button" href="' . $pub['slides_ppt'] . '">' . __('PPT', 'wpap') . '</a>';
					array_push($links, $link);
				}
				if (!empty($pub['website_url'])) {
					$link = '<a class="publication-button publication elementor-button button" href="' . $pub['website_url'] . '">' . __('Website', 'wpap') . '</a>';
					array_push($links, $link);
				}
				if (!empty($pub['DOI'])) {
					$link = '<a class="publication-button publication elementor-button button" href="https://doi.org/' . $pub['DOI'] . '">' . $pub['DOI'] . '</a>';
					array_push($links, $link);
				}
			...
			$body  	= '<div>' . $published_date->format( 'M d, Y' ) . '<br>';
			$body  .= '' . $pub['authors'] . '';

			$pubout = $header . $body . ((count($links) > 0) ? $links_str : '');

			$output .= '<li>' . $pubout . '</div></li>';
		...
		if (strtolower($options['numbered']) == 'true') {
			...
			$output = '<ul style="list-style: none;">' . $output . '</ul>';
		}
